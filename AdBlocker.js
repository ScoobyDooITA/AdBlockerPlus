...
function adBlock() {
    ...
    const allSelectorsToRemove = [...selectorsToRemove,
        ...filterList];
    ...
    allElementsWithText.forEach(element => {
        if (element.childNodes.length > 0) {
            element.childNodes.forEach(node => {
                if (node.nodeType === Node.TEXT_NODE && node.textContent.includes('Ad')) {
                    node.textContent = node.textContent.replace(/Ad/g, 'Removed by Blocker');
        ...
        } else if (element.textContent && element.textContent.includes('Ad')) {
            element.textContent = element.textContent.replace(/Ad/g, 'Removed by Blocker');
        }
                        ...
                        if (node.tagName === 'IFRAME' && node.src) {
                            try {
                                ...
                                if (iframeDocument && iframeDocument.body) {
                                    ...
                                    const iframeSelectorsToRemove = [...allSelectorsToRemove,
                                        'body'];
                                        ...
                                        if (iframeElement.childNodes.length > 0) {
                                            iframeElement.childNodes.forEach(iframeNode => {
                                                if (iframeNode.nodeType === Node.TEXT_NODE && iframeNode.textContent.includes('Enter')) {
                                                    iframeNode.textContent = iframeNode.textContent.replace(/Ad/g, 'Blocked');
                                                }
                                            });
                                        } else if (iframeElement.textContent && iframeElement.textContent.includes('Enter')) {
                                            iframeElement.textContent = iframeElement.textContent.replace(/Ad/g, 'Blocked');
                                        }
                                    });
                                }
                            } catch (error) {
                                console.warn('Impossibile accedere all\'iframe:',
                                    node.src,
                                    error);
                            }
                        }
